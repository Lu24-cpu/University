...
func CalcQuanteMinMax(frase string) (quante, max, min int) {
	if len(frase) == 0 {
		return 0, 0, 0
	}

	frasepulita := strings.ReplaceAll(frase, ",", "")
	frasepulita = strings.ReplaceAll(frasepulita, ":", "")
	parole := strings.Split(frasepulita, " ")

	for i, parola := range parole {
		_, err := strconv.Atoi(parola)

		if err != nil {
			if i == 0 {
				min, max = len(parola), len(parola)
			}
			if max < len(parola) {
				max = len(parola)
			}
...
func TrovaNumDopoKeyword(frase, keyWord string) (num int) {
	...
	for i := 0; i < len(parole)-1; i++ {
		if strings.Contains(parole[i], keyWord) {
			num, _ = strconv.Atoi(parole[i+1])
			return
		}
	}
	return
...
func Autogramma(frase string) bool {
	var parolechiave [3]string = [3]string{"contiene:", "massima:", "minima:"}
	var indicifunc []int
	var indiciveri [3]int

	for i := 0; i < 3; i++ {
		indicifunc = append(indicifunc, TrovaNumDopoKeyword(frase, parolechiave[i]))
	}
	indiciveri[0], indiciveri[1], indiciveri[2] = CalcQuanteMinMax(frase)

	var flag bool

	for i, elementi := range indiciveri {
		if indicifunc[i] == elementi {
			flag = true
		} else {
			flag = false
			break
...
func StampaAnalisiAutogramma(frase string) {
	var parolechiave [3]string = [3]string{"minima:", "massima:", "contiene:"}
	q, max, min := CalcQuanteMinMax(frase)

	fmt.Println("===", frase)
	fmt.Println("minimo dichiarato", TrovaNumDopoKeyword(frase, parolechiave[0]), "contro minimo verificato", min)
	fmt.Println("massimo dichiarato", TrovaNumDopoKeyword(frase, parolechiave[1]), "contro massimo verificato", max)
	fmt.Println("numero di parole dichiarato", TrovaNumDopoKeyword(frase, parolechiave[2]), "contro numero di parole verificato", q)
...
func main() {
	...
	for scanner.Scan() {
		frasi = append(frasi, scanner.Text())
	}
