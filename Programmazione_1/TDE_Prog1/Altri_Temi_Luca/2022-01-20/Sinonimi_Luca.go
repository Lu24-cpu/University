...
func main() {
	...
	dizionarionome, testonome := os.Args[1], os.Args[2]

	testofile, err := os.Open(testonome)
	defer testofile.Close()
	if err != nil {
		fmt.Println("Manca il file di testo")
	}
	dizionariofile, err := os.Open(dizionarionome)
	defer dizionariofile.Close()
	if err != nil {
		fmt.Println("Manca il file del dizionario")
	}

	fmt.Println("Testo originale: ")
	scanner := bufio.NewScanner(testofile)
	for scanner.Scan() {
		testo = append(testo, scanner.Text())
		fmt.Println(scanner.Text())
	}

	scanner1 := bufio.NewScanner(dizionariofile)
	for scanner1.Scan() {
		parole := strings.Split(scanner1.Text(), ": ")

		sinonimi[parole[0]] = strings.Split(parole[1], " ")
	}
	...
	for _, riga := range testo {
		...
		for _, parola := range parole {
			fmt.Print(parola)
			if sinonimi[parola] != nil {
				fmt.Print(sinonimi[parola])
			}
			fmt.Print(" ")
		}
		fmt.Println()
