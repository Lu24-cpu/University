...
func NewRectangle(P1, P2 POINT) RECTANGLE {
	return RECTANGLE{POINT{min(P1.x, P2.x), min(P1.y, P2.y)}, POINT{max(P1.x, P2.x), max(P1.y, P2.y)}}
}
...
func (r RECTANGLE) String() (stringa string) {
	stringa = fmt.Sprintf("((%f, %f) (%f, %f))", r.pLL.x, r.pLL.y, r.pUR.x, r.pUR.y)
	return
}
...
func main() {
	...
	for scanner.Scan() {
		riga := scanner.Text()
		xy := strings.Split(riga, " ")

		if i == 0 {
			x1.x, _ = strconv.ParseFloat(xy[0], 64)
			x1.y, _ = strconv.ParseFloat(xy[1], 64)
		} else {
			x2.x, _ = strconv.ParseFloat(xy[0], 64)
			x2.y, _ = strconv.ParseFloat(xy[1], 64)
		}

	}

	r := NewRectangle(x1, x2)
	fmt.Println(r)
}
