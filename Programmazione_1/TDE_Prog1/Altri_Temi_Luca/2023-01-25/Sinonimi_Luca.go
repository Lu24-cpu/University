...
func main() {
	var dizsinonimi, testi string
	var righe []string

	sin := make(map[string][]string)
	...
	filesin, err1 := os.Open(dizsinonimi)
	filetext, err2 := os.Open(testi)
	...
	testo := bufio.NewScanner(filetext)
	sinonimi := bufio.NewScanner(filesin)

	for testo.Scan() {
		righe = append(righe, testo.Text())
	}

	for sinonimi.Scan() {
		parts := strings.Split(sinonimi.Text(), ": ")
