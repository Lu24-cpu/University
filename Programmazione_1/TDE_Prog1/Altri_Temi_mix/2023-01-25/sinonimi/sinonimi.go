...
Per semplicità, basta che il testo in output mantenga gli a-capo del file da elaborare, non occorre invece che mantenga eventuali tab o spaziature multiple.
...
L'elaborazione consiste nel produrre un nuovo testo in cui tutte le parole che possiedono un sinonimo (secondo il file dei sinonimi) vengono stampate seguite dai loro sinonimi racchiusi tra parentesi quadre e in ordine alfabetico, ad esempio:
...
func Analyze_Dictionary(fname string)(map[string][]string,bool){
	...
	for scanner.Scan(){
		riga := scanner.Text()
		divisore := strings.Index(riga, ":")
		key := riga[:divisore]
		for _, word := range strings.Split(riga[divisore+2:], ","){
			dict[key] = append(dict[key], strings.ReplaceAll(word, " ", ""))
		}
		slices.Sort(dict[key])
	}
	return dict, true
}

func Get_Text(fname string, dictionary map[string][]string) (string, bool){
	var new_text string
	fp, err := os.Open(fname)
	if err != nil{
	...
	for scanner.Scan(){
		riga := scanner.Text()
		len_riga := len(riga)
		for i, current_word := range strings.Fields(riga){
			new_text += current_word
			if dictionary[current_word] != nil{
				new_text += fmt.Sprint(dictionary[current_word])
			}
			if i < len_riga-1{
				new_text += " "
			}
...
func main(){
	...
	var ok bool
	var testo string
	var dizionario_sinonimi map[string][]string
	dizionario_sinonimi, ok = Analyze_Dictionary(os.Args[1])

	if !ok{
		return
	}
	testo, ok = Get_Text(os.Args[2], dizionario_sinonimi)
	if !ok{
		return
	} 
	fmt.Println(testo)
